...
net = as_sfnetwork(carreteras) # ORIGINAL
...
# https://luukvdmeer.github.io/sfnetworks/articles/preprocess_and_clean.html#rounding-coordinates


# Dealing with one-way edges ----------------------------------------------
# https://luukvdmeer.github.io/sfnetworks/articles/preprocess_and_clean.html#dealing-with-one-way-edges


# Network cleaning functions ----------------------------------------------
# https://luukvdmeer.github.io/sfnetworks/articles/preprocess_and_clean.html#network-cleaning-functions
...
net = as_sfnetwork(carreteras, directed = FALSE) %>%
        # st_transform(4326) %>% # No es necesaria esta linea, pero si esta en otro CRS, dejar en 4326.
        activate("edges") %>%
...
paths = st_network_paths(net, from = 100, to = 1)
paths
...
par(mar = c(1,1,1,1))
plot(net, col = "grey")
plot(slice(activate(net, "nodes"), paths), col = "red", add = TRUE)
par(opar)
